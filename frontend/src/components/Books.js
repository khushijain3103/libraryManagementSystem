...
const Books = () => {   
  ...
  function fetchData()
  {
    fetch('https://www.googleapis.com/books/v1/volumes?q=react&key=AIzaSyB5a0vPyyTQrac6_K4cDIX0BO0z7Okva-I')
    .then((res) => res.json())
    .then((data) => {
       console.log(data);
      //  setPosts(data);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginRight: '50px', marginTop: '20px'}}>
        {Array.from({ length: Math.ceil(bookData?.totalItems / booksPerPage) })?.map((_, index) => (
